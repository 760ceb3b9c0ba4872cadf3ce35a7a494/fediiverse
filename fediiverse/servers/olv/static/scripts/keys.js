...
forceAllKeysDown();  // init all to false


function maybeReload() {
    var isPressed = (
        (cave.lls_getItem("reload-bind") == "ZL+ZR")
        ? (isKeyDown("ZL") && isKeyDown("ZR"))
        : (isKeyDown("L") && isKeyDown("R"))
    )
    ...
    if (isPressed) {
        ...
        cave.dialog_beginWait(null, "Reloading...");
    }
}

addKeyEventListener("L", maybeReload);
addKeyEventListener("R", maybeReload);
addKeyEventListener("ZL", maybeReload);
addKeyEventListener("ZR", maybeReload);